...
class XKTViewer extends Component {
  componentDidMount() {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (prevProps.models !== currentProps.models) {
      ...
      const perfModels = toAdd.map(el => this.XKTLoader.load(el));
  ...
  loadModels() {
    ...
    return models.map(model => this.XKTLoader.load(model));
  }
